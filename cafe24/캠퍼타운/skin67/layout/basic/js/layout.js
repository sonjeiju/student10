...
jQuery(document).ready(function() {
	...
	/* morenvy.com 카테고리가 대량일때 이상일때 출력버튼
	...
	/* morenvy.com 최상단배너 닫기버튼 없을시 높이 수정 */
	...
	/* morenvy.com 기획전 레이아웃변경에 따른 타겟고정위치 변경 */
	...
	/* morenvy.com 슬라이드 카테고리 클릭시 */
    jQuery('#aside ul a.cate').on('click', function(e){
		jQuery(this).parent().find('li').removeClass('selected');
		jQuery(this).parent().toggleClass('selected');
		if (!jQuery(this).parent('li').hasClass('noChild')){
			e.preventDefault();
		}
    });
	...
	top_category(); // 상단카테고리
	jQuery('.xans-layout-category > ul').on('DOMSubtreeModified', function() {
		top_category();
	});

	/* morenvy.com 상단 카테고리 중분류 마우스오버 */
	jQuery('.xans-layout-category > ul > li .sub-category li').mouseenter(function() {
...
function top_category(){
	...
	/* morenvy.com 상단 카테고리 확장형 */
	jQuery('.xans-layout-category > ul > li').each(function(){
		if( jQuery(this).children('ul').length == 0 ){
		} else {
